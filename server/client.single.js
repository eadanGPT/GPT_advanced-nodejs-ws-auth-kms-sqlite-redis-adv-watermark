...
const ws = new WebSocket(CONFIG.url, {
  ...
  ws.on('message', async (raw)=>{
    ...
    if (msg.typ === 'hello') {
      ...
      // Fetch & verify signed module manifest
      try {
        const pinned = PinStore.get()?.pubKeyB64;
        globalThis.__MODULE_MANIFEST = await __fetchAndVerifyManifest(ws, sendAndWait, pinned);
      } catch (e) {
        console.error('Manifest verification failed:', e?.message || e);
        // Optionally abort here:
        // throw e;
      }

      // Verify client binary against server-published hash
      try {
        await __verifySelfBinary(sendAndWait);
      } catch (e) {
        console.error('Self binary verification failed:', e?.message || e);
        // Optionally abort here:
        // throw e;
      }
...
// === Ephemeral string usage helper (best-effort zeroization) ===
function __withEphemeralBuffer(str, fn) {
  if (typeof str !== 'string') return fn(null);
...
// === Signed manifest fetch/verify ===
async function __fetchAndVerifyManifest(ws, sendAndWait, pinnedServerKeyB64) {
  const resp = await sendAndWait({ action: 'get_manifest' });
  const { manifest, sig } = (resp && resp.result) || {};
  if (!manifest) throw new Error('manifest_missing');

  const canon = JSON.stringify(manifest, Object.keys(manifest).sort());
  if (sig && pinnedServerKeyB64) {
    try {
      // Expect pinnedServerKeyB64 to be base64 DER SPKI of Ed25519
      const ok = crypto.verify(null, Buffer.from(canon), {
        key: Buffer.from(pinnedServerKeyB64, 'base64'),
        format: 'der',
        type: 'spki'
      }, Buffer.from(sig, 'base64'));
      if (!ok) throw new Error('manifest_sig_invalid');
    } catch (e) {
...
// === Self-binary verification ===
async function __verifySelfBinary(sendAndWait) {
  // Compute hash of running script/binary
  const selfPath = process.argv[1];
  let localHash = null;
  try {
    const data = fs.readFileSync(selfPath);
    localHash = crypto.createHash('sha256').update(data).digest('hex');
  } catch {}
  const resp = await sendAndWait({ action: 'get_client_hash' });
  const serverHash = resp?.result?.clientHash || null;
  if (!serverHash || !localHash || serverHash !== localHash) {
    throw new Error('client_self_hash_mismatch');
  }
  return true;
}


// === Verify module bytes against manifest hash ===
function __verifyModuleAgainstManifest(moduleName, bytes) {
  try {
    const man = globalThis.__MODULE_MANIFEST || {};
    const expect = man?.[moduleName]?.sha256;
    if (!expect) return true; // if not listed, skip
    const got = crypto.createHash('sha256').update(bytes).digest('hex');
    if (expect !== got) throw new Error(`module_integrity_mismatch:${moduleName}`);
    return true;
  } catch (e) { throw e; }
}
